...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const login = async (email: string, password: string) => {
    ...
    setError(null);
    try {
      const res = await fetch("https://lexxes-group-backend.onrender.com/api/auth/login", {
        method: "POST",
  ...
  const register = async (formData: RegisterData) => {
    ...
    setError(null);
    try {
      const res = await fetch("https://lexxes-group-backend.onrender.com/api/auth/register", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
