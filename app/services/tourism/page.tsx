...
const destinations = [
  { img: "/pageimages/tourism/jaipur.png", place: "Jaipur, India", tag: "Starter Reward", desc: "The Pink City — royal palaces, vibrant bazaars and Rajput grandeur.", highlights: ["5 Days / 4 Nights", "Heritage Hotel Stay", "Sightseeing Tours", "Group Activities"] },
...
const packages = [
  {
    ...
    border: "border-slate-200", badge: "bg-amber-100 text-amber-700 border-amber-200",
  ...
  },
  {
    ...
    border: "border-gold-primary/30", badge: "bg-gold-primary/10 text-gold-primary border-gold-primary/20", popular: true,
...
export default function TourismPage() {
  ...
  return (
    <main className="min-h-screen bg-[#FAFAF8] text-navy-dark overflow-x-hidden selection:bg-gold-primary selection:text-navy-dark">
      ...
      {/* ══════════════════════════════════════
          HERO — FULL SCREEN, NAVBAR OVERLAPS
      ══════════════════════════════════════ */}
      <section className="relative h-screen min-h-[700px] overflow-hidden">
        ...
        {heroSlides.map((slide, i) => (
          <div key={i} className={`absolute inset-0 transition-opacity duration-1000 ${current === i ? "opacity-100 z-10" : "opacity-0 z-0"}`}>
            ...
            />
            <div className="absolute inset-0 bg-gradient-to-t from-[#0a0f1e] via-[#0a0f1e]/50 to-[#0a0f1e]/10" />
        ...
        <div className="absolute inset-0 z-20 flex flex-col justify-center px-6 md:px-12 lg:px-20 pt-20">
          <div key={`slide-${current}`} className="max-w-4xl">
            ...
            <div className="fade-up inline-flex items-center gap-3 mb-6 px-4 py-2 rounded-full border border-white/20 bg-white/10 backdrop-blur-sm">
              <Plane size={13} className="text-gold-primary" />
              <span className="font-inter text-[10px] font-bold tracking-[0.3em] text-white uppercase">Travel Asia • Tourism</span>
            </div>

            <h1 className="fade-up delay-1 font-playfair text-5xl md:text-7xl lg:text-[5.5rem] font-bold text-white mb-4 leading-[1.0] tracking-tight">
              Your Success,<br />
            ...
            <div className="fade-up delay-2 flex items-center gap-2 mb-3">
              <MapPin size={14} className="text-gold-primary" />
              <span className="font-inter text-sm text-white/90 font-semibold">{heroSlides[current].place}</span>
            ...
            <div className="fade-up delay-3 flex flex-col sm:flex-row gap-4 mb-12">
              <Link href="/register" className="group relative flex items-center justify-center gap-3 bg-gold-primary text-navy-dark px-8 py-4 font-inter font-bold text-xs uppercase tracking-widest transition rounded-xl shadow-[0_4px_20px_rgba(201,168,76,0.4)] overflow-hidden w-fit">
      ...
      {/* ══════════════════════════════════════
          SECTION 2 — WHY TRAVEL WITH LEXXES
      ══════════════════════════════════════ */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div>
              ...
              <h2 className="font-playfair text-4xl md:text-5xl font-bold mb-6 leading-tight">
                ...
                <span className="text-gold-primary italic">It's your reward.</span>
              </h2>
              <p className="font-inter text-base text-white/60 font-light leading-relaxed mb-8 max-w-lg">
              ...
              <div className="flex flex-col gap-5">
                {[
                  ...
                  <div key={item.title} className="flex items-start gap-4">
                    <div className="w-10 h-10 rounded-xl bg-gold-primary/10 border border-gold-primary/20 flex items-center justify-center shrink-0 mt-0.5">
                      <item.icon size={17} className="text-gold-primary" />
                    </div>
                    <div>
                      <p className="font-inter font-bold text-sm text-white mb-1">{item.title}</p>
                      <p className="font-inter text-sm text-white/50 font-light">{item.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
              <Link href="/register" className="group mt-10 inline-flex items-center gap-3 bg-gold-primary hover:bg-gold-light text-navy-dark px-8 py-4 font-inter font-bold text-xs uppercase tracking-widest transition rounded-xl shadow-[0_4px_20px_rgba(201,168,76,0.3)]">
      ...
      {/* ══════════════════════════════════════
          SECTION 3 — DESTINATIONS GALLERY
      ══════════════════════════════════════ */}
              ...
              <div key={i} className="group relative overflow-hidden rounded-[2rem] bg-white border border-slate-100 shadow-sm h-[460px] cursor-pointer">
                ...
                <div className="absolute bottom-4 left-4 right-4 bg-white/90 backdrop-blur-xl border border-white/60 p-5 rounded-2xl translate-y-1 group-hover:translate-y-0 transition-transform duration-500">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="font-inter text-[9px] font-bold uppercase tracking-wider px-2 py-1 rounded-md bg-amber-50 text-amber-700 border border-amber-200">{dest.tag}</span>
                  </div>
      ...
      {/* ══════════════════════════════════════
          PACKAGES
      ══════════════════════════════════════ */}
      <section id="packages" className="py-24 bg-white border-y border-slate-100">
        <div className="max-w-7xl mx-auto px-6 md:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 max-w-5xl mx-auto">
            {packages.map((pkg) => (
              <div key={pkg.name} className={`group relative flex flex-col rounded-[2.5rem] border ${pkg.border} bg-white shadow-[0_20px_60px_rgba(15,23,42,0.06)] overflow-hidden hover:-translate-y-2 transition-transform duration-500`}>
                {pkg.popular && (
                  <div className="absolute top-6 right-6 z-20 px-4 py-1.5 bg-gold-primary text-navy-dark font-inter font-bold text-[9px] uppercase tracking-widest rounded-full">Most Popular</div>
                )}
                <div className="relative h-64 overflow-hidden rounded-[2rem] m-4">
                  ...
                  <div className="absolute bottom-6 left-6">
                    ...
                    <span className={`inline-block px-3 py-1 rounded-lg border text-[9px] font-bold uppercase tracking-widest font-inter backdrop-blur-md ${pkg.badge}`}>{pkg.name}</span>
                  </div>
                </div>
      ...
      <section className="py-24 bg-navy-dark text-white">
        <div className="max-w-7xl mx-auto px-6 md:px-8">
          <div className="mb-14">
            ...
            <h2 className="font-playfair text-4xl md:text-5xl font-bold">Earn Your Trip in <span className="text-gold-primary italic">4 Steps.</span></h2>
          </div>
      ...
      {/* ══════════════════════════════════════
          TESTIMONIALS
      ══════════════════════════════════════ */}
      <section className="py-24 bg-amber-50/60 border-t border-amber-100">
        <div className="max-w-7xl mx-auto px-6 md:px-8">
          <div className="mb-14 text-center">
            <p className="font-inter text-[10px] tracking-[0.3em] text-amber-600 uppercase mb-3 font-bold flex items-center justify-center gap-2">
              <span className="w-8 h-px bg-amber-300" /> Member Stories <span className="w-8 h-px bg-amber-300" />
            </p>
            <h2 className="font-playfair text-4xl md:text-5xl font-bold text-navy-dark">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              <div key={t.name} className="p-8 rounded-[2rem] bg-white border border-amber-100 shadow-sm hover:border-amber-200 transition-colors flex flex-col">
                <div className="flex gap-0.5 mb-5">
                  {Array(5).fill(0).map((_, i) => <Star key={i} size={14} className="text-amber-400 fill-amber-400" />)}
                </div>
                <p className="font-playfair text-lg text-slate-700 font-light leading-relaxed mb-6 italic flex-1">"{t.quote}"</p>
                <div className="flex items-center gap-3 pt-5 border-t border-amber-50">
                  <div className="w-10 h-10 rounded-full bg-amber-100 border border-amber-200 flex items-center justify-center font-playfair font-bold text-base text-amber-700">{t.name[0]}</div>
                  <div>
                    <p className="font-inter font-bold text-sm text-navy-dark">{t.name}</p>
                    <p className="font-inter text-[9px] font-bold text-amber-600 uppercase tracking-widest mt-0.5">{t.location} • {t.trip}</p>
                  </div>
                </div>
              </div>
            ))}
